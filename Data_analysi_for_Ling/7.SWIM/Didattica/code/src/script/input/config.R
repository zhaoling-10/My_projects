config <- function(){
  ...
  filename_CTRL <- paste0(path,"/list/LE.txt")
  filename_CASE <- paste0(path,"/list/LT.txt")
  ...
  paired_ttest <- F            # a logical value indicating whether you want a paired t-test
  ...
  #threshold_prc_corr <- 0.45 # Lepus timidus
  threshold_prc_corr <- 0.65 # Lepus europaeus
  ...
  # <<< NUOVO FLAG >>>
  # TRUE  = centra per genotipo (come in 2x vs 4x)
  # FALSE = nessuna centratura per genotipo (come leaf vs inflorescence)
  use_genotype_center <- FALSE
  
  # Per il dataset ploidy 2x vs 4x, puoi usare lo stesso script ma con:
  # use_genotype_center <- TRUE
  ...
  input_parameter <- list(path = path,
                          ...
                          use_genotype_center = use_genotype_center)
