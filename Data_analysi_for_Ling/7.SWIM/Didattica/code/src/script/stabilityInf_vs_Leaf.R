...
run_stability <- function(data,
  ...
  if (!is.null(input_file$metadata)) {
    ...
    # Se non hai un file di metadata, la ploidy viene dedotta da control/case
    # (per il dataset leaf vs inflorescence questi nomi non contano molto)
    ...
    # Il "genotype" di fallback viene dal prefisso del nome sample
    # (es: "leaf_1" -> "leaf", "inflorescence_1" -> "inflorescence")
