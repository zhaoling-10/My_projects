...
run_stability <- function(
    ...
    fixed_threshold = TRUE,                  # use the reference threshold across all refits
    # edge-overlap option: 0 = use all edges; >0 = use top M edges by |r|
    topM_edges = 0,
    # sensitivity knobs
    corr_type = c("pearson","spearman"),     # default pearson
    threshold_shift = -0.03,                 # slightly more lenient than SWIM’s auto-threshold
    threshold_min   = 0.45                   # minimum threshold floor
) {
  nesting_mode <- match.arg(nesting_mode)
  corr_type    <- match.arg(corr_type)
  message("==> Stability diagnostics: start (nesting_mode = ", nesting_mode,
  ...
  if (nesting_mode == "residuals") {
    # regress out genotype; keep residuals (replicate-level correlations)
  ...
  } else { # "average": average replicates within genotype (genotype-level correlations)
  ...
  run_once <- function(subset_samples, case_now, ctrl_now, thr_override = NULL) {
    ...
    thr <- max(threshold_min, thr + threshold_shift)  # apply shift and floor
