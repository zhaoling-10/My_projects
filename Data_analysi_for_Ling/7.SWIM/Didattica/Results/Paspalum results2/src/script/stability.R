...
run_stability <- function(
    ...
    n_subsamples = 200, n_perm = 1000, seed = 123,
    nesting_mode = c("residuals","average"),
    verbose = FALSE, progress = TRUE, fixed_threshold = FALSE,
    # permutation checkpointing
    perm_checkpoint_file = "perm_checkpoint.rds", checkpoint_every = 10, resume_perm = TRUE,
    # edge-overlap option: 0 = use all edges; >0 = use top M edges by |r|
    topM_edges = 0,
    # NEW sensitivity knobs
    corr_type = c("pearson","spearman"),
    threshold_shift = 0,            # e.g., -0.05 to be slightly lenient
    threshold_min   = 0.45          # do not go below this threshold
) {
  nesting_mode <- match.arg(nesting_mode)
  corr_type    <- match.arg(corr_type)
  message("==> Stability diagnostics: start")
  ...
  # ----- infer genotype IDs from naming scheme -----
  ...
  } else { # averaging replicates per genotype
  ...
  if (!all(keep_genes)) {
    expr_for_corr <- expr_for_corr[keep_genes, , drop = FALSE]
  }
  
  # ----- override CASE/CTRL per run -----
  ...
  # Jaccard over all edges or over top-M by |r|
  topM_edge_keys <- function(data_input, M, thr) {
    ...
    if (M <= 0) {
      ...
      # rank by |r| among edges above threshold, upper triangle only
      Rabs <- abs(R)
      Rabs[!A] <- 0
  ...
  # ----- run_once: rebuild adjacency from data_input using (shifted) SWIM threshold -----
  .thr_log <- numeric(0)
  ...
  run_once <- function(subset_samples, case_now, ctrl_now, thr_override = NULL) {
    ...
    # apply shift and floor
    thr <- max(threshold_min, thr + threshold_shift)
    
    if (verbose) message("Using threshold_corr = ", signif(thr, 3), " (", corr_type, ") to rebuild adjacency")
  ...
  # (frequency a gene is called a switch across all (1 + nrow(robust_df)) runs)
  ...
  if (length(all_switches) > 0) {
    ...
    # create empty file to be explicit
  ...
  # ----- 6) Permutation null (checkpointable) -----
  obs_count <- length(ref_switches)
  message("-> Permutation null (", n_perm, " runs)...")
  
  start_idx <- 1
  perm_counts <- rep(NA_real_, n_perm)
  
  if (resume_perm && file.exists(perm_checkpoint_file)) {
    ck <- tryCatch(readRDS(perm_checkpoint_file), error = function(e) NULL)
    if (!is.null(ck) && length(ck$perm_counts) == n_perm) {
      perm_counts <- ck$perm_counts
      last_done <- max(which(!is.na(perm_counts)), 0)
      start_idx <- last_done + 1
      message(sprintf("Resuming permutation at run %d (of %d).", start_idx, n_perm))
    }
  }
  
  if (progress) { pb_perm <- txtProgressBar(min = 0, max = n_perm, style = 3); on.exit({ if (exists("pb_perm")) close(pb_perm) }, add = TRUE) }
  
  if (start_idx <= n_perm) {
    for (r in start_idx:n_perm) {
      set.seed(seed + 2000 + r)
      pl_perm <- sample(ploidy)
      case_now <- sample_ids[pl_perm == "4x"]
      ctrl_now <- sample_ids[pl_perm == "2x"]
      fr <- run_once(
        subset_samples = sample_ids,
        case_now = case_now, ctrl_now = ctrl_now,
        thr_override = if (fixed_threshold) ref_thr else NULL
      )
      perm_counts[r] <- length(unique(fr$switches))
      
      if (checkpoint_every > 0 && (r %% checkpoint_every == 0 || r == n_perm)) {
        saveRDS(list(perm_counts = perm_counts,
                     done = r, seed = seed,
                     nesting_mode = nesting_mode,
                     ref_thr = ref_thr, fixed_threshold = fixed_threshold,
                     thr_log = .thr_log),
                file = perm_checkpoint_file)
      }
      if (progress) setTxtProgressBar(pb_perm, r)
    }
  } else {
    message("All permutation runs were already completed in checkpoint.")
  }
  
  # Save permutation distribution and p-value
  emp_p <- (sum(perm_counts >= obs_count, na.rm = TRUE) + 1) / (sum(!is.na(perm_counts)) + 1)
  write.csv(data.frame(run = seq_len(n_perm), switch_count = perm_counts),
            "Permutation_Distribution.csv", row.names = FALSE)
  writeLines(sprintf("Observed_switches = %d\nEmpirical_p = %.6g\nCompleted_runs = %d",
                     obs_count, emp_p, sum(!is.na(perm_counts))),
             "Permutation_Summary.txt")
  
  pdf("Supplementary_Fig_PermutationNull.pdf", width = 6, height = 4)
  hist(perm_counts, breaks = 30, main = "Permutation Null: # Switch Genes",
       xlab = "Switch genes (permuted labels)")
  abline(v = obs_count, lwd = 2)
  mtext(sprintf("Observed = %d; Empirical p = %.4g", obs_count, emp_p), side = 3, line = 0.5, cex = 0.9)
  dev.off()
  
  saveRDS(list(perm_counts = perm_counts, observed = obs_count, p = emp_p,
               nesting_mode = nesting_mode, thr_log = .thr_log),
          file = "PermutationNull_switchCount.rds")
  
  if (all(!is.na(perm_counts)) && file.exists(perm_checkpoint_file)) file.remove(perm_checkpoint_file)
