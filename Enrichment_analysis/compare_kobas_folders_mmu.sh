#!/usr/bin/env bash
# =============================================================================
# KOBAS Output Folder Comparison — Mus musculus (mmu) only
# Compares your mmu KOBAS results vs colleague's (6 pairs: GO/KEGG × 508/158/133)
# Standalone script; does not modify compare_kobas_folders.sh
# =============================================================================
...
# ---- 6 mmu folder pairs: my_folder|colleague_folder|analysis_type|label ----
...
REPORT="${MY_BASE}/kobas_comparison_report_mmu_$(date +%Y%m%d_%H%M%S).txt"
...
log ""
hr
log "${BOLD}  KOBAS OUTPUT COMPARISON REPORT (mmu — 6 folder pairs)${NC}"
log "  Generated: $(date)"
log "  Script:    compare_kobas_folders_mmu.sh"
hr

log ""
log "${BOLD}[0] FOLDER EXISTENCE CHECK (6 mmu pairs)${NC}"
log ""
all_exist=1
...
for entry in "${FOLDER_PAIRS[@]}"; do
    ...
    PAIR_NUM=$((PAIR_NUM + 1))
    log ""
    log "${BOLD}--- Pair ${PAIR_NUM}/6: ${set_label} ---${NC}"
